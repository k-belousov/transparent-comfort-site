...
const GalleryCard = ({ item, index, showIndicator, currentSlide }: { item: typeof galleryItems[0], index: number, showIndicator: boolean, currentSlide: number }) => {
      ...
      <DialogTrigger asChild>
        ...
        >
          ...
          <div className="relative h-64 overflow-hidden">
            ...
            {/* Премиальная иконка увеличения при наведении и при автопереключении */}
            <div className={`absolute inset-0 flex items-center justify-center transition-all duration-500 ${isHovered || (showIndicator && index === currentSlide) ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}>
            ...
            {/* Мобильный индикатор касания - УДАЛЕН */}
            ...
            <CarouselContent>
              {galleryItems.map((item, index) => (
                <CarouselItem key={item.id} className={`md:basis-1/2 lg:basis-1/3 ${contentVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: `${index * 100}ms` }}>
                  <GalleryCard item={item} index={index} showIndicator={showIndicator && index === current} currentSlide={current} />
                </CarouselItem>
              ))}
            </CarouselContent>
